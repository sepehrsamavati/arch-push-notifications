...
import { VapidHelper2 } from './VapidHelper.js';
import { UserSubscription } from '../repository/mongo/fakeRepo.js';
import { isCryptoKey } from "node:util/types";

export class EncryptionAES128GCM {
	private _b64ServerKeys;
	private _b64Salt;
	private _b4VapidKeys;

	constructor(options: { serverKeys?: { publicKey: string; privateKey: string; }; salt?: string; vapidKeys?: string; } = {}) {
	...
	async getRequestDetails(subscription: UserSubscription, payloadText: string) {
		...
		if (vapidHeaders) {
			Object.entries(vapidHeaders).forEach(([headerName, headerValue]) => {
	...
	async encryptPayload(subscription: UserSubscription, payloadText: string) {
		...
		const contentEncryptionCryptoKey = await crypto.subtle.importKey('raw',
			...
			['decrypt', 'encrypt']);
		encryptionKeys.contentEncryptionKey = contentEncryptionCryptoKey as unknown as Buffer;
		...
		const encryptedPayloadArrayBuffer = await crypto.subtle.encrypt(
			algorithm, encryptionKeys.contentEncryptionKey as unknown as CryptoKey,
			recordUint8Array,
		);
	...
	async _addEncryptionContentCodingHeader(
		encryptedPayloadArrayBuffer: ArrayBuffer, serverKeys: any, salt: Uint8Array) {
	...
	async _generateEncryptionKeys(subscription: UserSubscription, salt: Uint8Array, serverKeys: any) {
		...
		const cekHKDF = new HKDF(Buffer.from(prk), Buffer.from(salt));
		const nonceHKDF = new HKDF(Buffer.from(prk), Buffer.from(salt));
		const keyResults = await Promise.all([
			cekHKDF.generate(Buffer.from(cekInfo), 16),
			nonceHKDF.generate(Buffer.from(nonceInfo), 12),
		]);
		return {
	...
	async _generatePRK(subscription: UserSubscription, serverKeys: any) {
		...
		const hkdf = new HKDF(
			...
			Buffer.from(subscription.auth),
		);
		return hkdf.generate(Buffer.from(keyInfoUint8Array), 32);
	}

	async _getSharedSecret(subscription: UserSubscription, serverKeys: any) {
		// const keys = await arrayBuffersToCryptoKeys(
		// 	Uint8Array.from(
		// 		Buffer.from(subscription.p256dh)
		// 	)
		// );
		const keys = {
			publicKey: serverKeys.publicKey
		};
		if (!isCryptoKey(keys.publicKey)) {
	...
	async _getKeyInfo(subscription: UserSubscription, serverKeys: any) {
		const utf8Encoder = new TextEncoder();
		...
		return joinUint8Arrays([
			...
			new Uint8Array(Buffer.from(subscription.p256dh)),
			keyInfo.publicKey,
		]);
	}
}