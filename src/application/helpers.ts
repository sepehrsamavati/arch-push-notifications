...
export function uint8ArrayToBase64Url(uint8Array: Uint8Array, start?: number, end?: number) {
	...
	const base64 = Buffer.from(uint8Array.subarray(start, end)).toString('base64');
	return base64
...
export function base64UrlToUint8Array(base64UrlData: string) {
	...
	const buffer = Uint8Array.from(rawData);
...
// as required.
export function joinUint8Arrays(allUint8Arrays: Uint8Array[]) {
...
export async function arrayBuffersToCryptoKeys(publicKey: Uint8Array, privateKey?: Uint8Array) {
...
export async function cryptoKeysToUint8Array(publicKey: any, privateKey?: any) {
	const promises = [];
	...
	if (privateKey) {
		...
		promises.push(
			base64UrlToUint8Array(jwk.d as string),
		);
	}
